...
class UR4RecV2MoE(nn.Module):
    ...
    def __init__(
        ...
        # ===========================
        # [改进] Router输入：对齐后的多模态特征拼接 (visual + text)
        # 原因：让Router能"看到"多模态特征，判断应该依赖哪个Expert
        self.router = ItemCentricRouter(
            item_emb_dim=sasrec_hidden_dim * 2,  # 256 = 128(visual_aligned) + 128(text_aligned)
    ...
    def forward(
        ...
        # ===========================
        # 4.1 [改进] 构建Router的多模态输入
        # ===========================
        # Router输入：对齐后的多模态特征拼接，让Router能"看到"特征质量
        if training_mode:
            # 训练模式: [B, visual_dim] 和 [B, text_dim]

            # 1. 获取对齐后的视觉特征
            if target_visual is not None:
                if target_visual.dim() == 3:  # [B, 1, 512]
                    target_visual_single = target_visual[:, 0, :]  # [B, 512]
                else:  # [B, 512]
                    target_visual_single = target_visual
                visual_aligned = self.visual_proj(target_visual_single)  # [B, 128]
            else:
                visual_aligned = torch.zeros(batch_size, self.sasrec_hidden_dim, device=self.device)  # [B, 128]

            # 2. 获取对齐后的文本特征（如果有target_text的话）
            # 注意：semantic expert使用ID嵌入作为query，但Router需要看到文本特征本身
            if target_text is not None:
                if target_text.dim() == 3:  # [B, 1, 384]
                    target_text_single = target_text[:, 0, :]  # [B, 384]
                else:  # [B, 384]
                    target_text_single = target_text
                text_aligned = self.text_proj(target_text_single)  # [B, 128]
            else:
                text_aligned = torch.zeros(batch_size, self.sasrec_hidden_dim, device=self.device)  # [B, 128]

            # 3. 拼接多模态特征作为Router输入
            router_input = torch.cat([visual_aligned, text_aligned], dim=-1)  # [B, 256]
            router_weights = self.router(router_input)  # [B, 2]

        else:
            # 推理模式: [B, N, visual_dim] 和 [B, N, text_dim]

            # 1. 获取对齐后的视觉特征
            if target_visual is not None:
                if target_visual.dim() == 3:  # [B, N, 512]
                    B, N, _ = target_visual.shape
                    target_visual_flat = target_visual.view(B * N, -1)  # [B*N, 512]
                    visual_aligned = self.visual_proj(target_visual_flat).view(B, N, -1)  # [B, N, 128]
                else:  # [B, 512] - 所有候选使用相同视觉特征
                    visual_aligned = self.visual_proj(target_visual).unsqueeze(1).expand(-1, num_candidates, -1)  # [B, N, 128]
            else:
                visual_aligned = torch.zeros(batch_size, num_candidates, self.sasrec_hidden_dim, device=self.device)

            # 2. 获取对齐后的文本特征
            if target_text is not None:
                if target_text.dim() == 3:  # [B, N, 384]
                    B, N, _ = target_text.shape
                    target_text_flat = target_text.view(B * N, -1)  # [B*N, 384]
                    text_aligned = self.text_proj(target_text_flat).view(B, N, -1)  # [B, N, 128]
                else:  # [B, 384] - 所有候选使用相同文本特征
                    text_aligned = self.text_proj(target_text).unsqueeze(1).expand(-1, num_candidates, -1)  # [B, N, 128]
            else:
                text_aligned = torch.zeros(batch_size, num_candidates, self.sasrec_hidden_dim, device=self.device)

            # 3. 拼接多模态特征作为Router输入
            router_input = torch.cat([visual_aligned, text_aligned], dim=-1)  # [B, N, 256]
            router_weights = self.router(router_input)  # [B, N, 2]
